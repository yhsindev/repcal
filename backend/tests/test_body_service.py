...
async def test_record_weight_creates_row(session: AsyncSession) -> None:
    user = await identity_service.get_or_create_user(session, Platform.DISCORD, "discord-user-123")
...
async def test_record_weight_same_day_overwrites(session: AsyncSession) -> None:
    ...
    user = await identity_service.get_or_create_user(session, Platform.DISCORD, "discord-user-456")
    assert user.id is not None
...
async def test_different_platforms_get_different_users(session: AsyncSession) -> None:
    ...
    discord_user = await identity_service.get_or_create_user(session, Platform.DISCORD, "12345")
